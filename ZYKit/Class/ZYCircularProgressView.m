...
@interface ZYCircularProgressView () {
    CGFloat oldFrameWidth;
}
@end

@implementation ZYCircularProgressView
...
- (instancetype) initWithFrame:(CGRect)frame
{
    ...
    if (self) {
        ...
        self.progressBackgroundMode = ZYProgressBackgroundModeCircumference;
        self.progressMode = ZYProgressModeFill;
        self.clockwise = YES;
...
- (void)drawBackground:(CGRect)rect
{
    switch (self.progressBackgroundMode) {
        case ZYProgressBackgroundModeCircle: {
        ...
        case ZYProgressBackgroundModeCircumference: {
        ...
        case ZYProgressBackgroundModeNone:
        default:
            break;
    }
}
...
- (void)drawProgress:(CGRect)rect
{
    ...
    if ((!self.clockwise && percentage < 1.0f) || (self.progressMode == ZYProgressModeDeplete && percentage == 0.0f)) {
        percentage = 1.0f - percentage;
    }

    BOOL clockwise = YES;
    if (percentage < 1.0f && ((self.clockwise && self.progressMode == ZYProgressModeDeplete) || (!self.clockwise && self.progressMode == ZYProgressModeFill))) {
        clockwise = NO;
    }
    ...
    if (self.progressMode == ZYProgressModeFill && self.percentage > 0) {
        [self drawProgressArcWithStartAngle:startAngle endAngle:endAngle radius:radiusMinusLineWidth clockwise:clockwise];
    }
    else if (self.progressMode == ZYProgressModeDeplete && self.percentage < 1) {
        [self drawProgressArcWithStartAngle:startAngle endAngle:endAngle radius:radiusMinusLineWidth clockwise:clockwise];
    }
